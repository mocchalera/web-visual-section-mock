...
- レイヤー出力時は、基準画像、再構成用レイヤー、再利用用素材、manifest、ゼロ差分証明を1つの契約として扱う
...
素材出力モード:
[composite_only / layer_plan_only / layered_exact]
...
レイヤー生成契約（layered_exactの場合）:
- canonical composite: [承認済み基準画像]
- reconstruction layers: [同一キャンバスまたは座標付きロスレスRGBA]
- reusable assets: [trim済み透過PNG、テキスト仕様、SVG/CSS候補]
- assembly manifest: [canvas、座標、z-order、opacity、blend mode、対応関係]
- fidelity gate: [decoded RGBAでpixel_diff_count = 0、max_channel_delta = 0]
- failure state: [一致を証明できない場合はfidelity_unprovenまたはblocked]
...
レイヤー出力時、canonical compositeを固定したか:
[yes/no/not-applicable]

reconstruction layersとreusable assetsを分けたか:
[yes/no/not-applicable]

assembly manifestで合成条件が一意か:
[yes/no/not-applicable]

decoded RGBAでpixel_diff_count = 0かつmax_channel_delta = 0か:
[yes/no/not-applicable]
...
現在の問題:
[見切れ / 詰め込み / 文字が小さい / 安っぽい / 暗い / タイポグラフィが弱い / AI破綻 / 再合成不一致 / 編集不能なレイヤー / その他]
...
- 安っぽいテンプレート感を消す
- レイヤー出力時はcanonical compositeを変えず、manifestどおり再合成する
- 再合成不一致はdecoded RGBAの差分座標から該当レイヤーを修正する
- フル画像コピーや大きなresidualで差分を隠さない
...
layer_ledger:
- [奥から手前のlayer_id、semantic_role、source_strategy、z_index]

generation_order:
1. 上層要素を含まない背景／下地プレート
2. 背景構造、色面、巨大モチーフ
3. 人物、商品、UI、カード
4. アイコン、手書き文字、矢印、下線、スタンプ
5. 読ませる文字とCTAの完全再現用ラスタースナップショット
6. 編集用テキスト仕様とSVG/CSS候補

reconstruction_outputs:
- [同一キャンバスまたは整数座標付きのロスレスRGBAレイヤー]
...
- blend_mode
- transform
- reconstruction layerとreusable assetの対応

verification:
- recompose manifestどおり
- compare decoded RGBA
- same_dimensions = true
- pixel_diff_count = 0
- max_channel_delta = 0
- full-frame copyでの偽装なし
- residualが意味領域を隠していない

status:
[complete_exact_editable / fidelity_unproven / reconstruction_mismatch / editability_failed / blocked_tooling]
