...
- 迷った場合は余白を選ぶ
- 各セクションでは、使う型を少数に絞る
- レイヤー出力時は、全素材の再合成が基準画像とデコード後RGBAで完全一致することを証明する
...
12. 画像生成プロンプトへ変換する
13. レイヤー出力が必要なら、基準画像固定、素材生成、manifest、再合成、ゼロ差分検証を行う
14. NGチェックを行う
```
...
9. レイヤー出力時は、基準画像と再合成画像がピクセル差分0であること
...
- 実際にレイヤー／パーツ素材を書き出す場合は、続けて `11_layered_asset_generation_and_reconstruction.md` を使い、完全再現用レイヤーと編集・実装用素材を分ける。
